...
_Ops: `[5B]`, `[5C]`, `[5D]`_ · _Source: `cop_handlers_lifecycle.asm` + `cop_handlers_map.asm`_
...
| `$80` + `CalcTileMapOffset` | Map collision layer pointer (`cop_handlers_map.asm`) |
...
- `$5D` handler lives in **`cop_handlers_map.asm`**, not lifecycle — same dispatch table, different compilation unit.
...
- **Handler:** `OrExtraFlags` @ `cop_handlers_lifecycle.asm:314-324`
...
- **Handler:** `AndExtraFlags` @ `cop_handlers_lifecycle.asm:329-339`
...
- **Handler:** `BranchIfBehindWall` @ `extracted/system/engine/cop_handlers_map.asm:570-622`
