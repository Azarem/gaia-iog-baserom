...
_Ops: `[D8]`, `[D9]`_ · _Source: `extracted/system/engine/cop_handlers_flow.asm`_

[← COP index](../index.md)

## Overview

Two flow handlers with different jobs: **`SetDungeonKillFlag`** persists per-dungeon enemy defeat bits in WRAM, and **`SwitchCase`** performs byte-indexed jump-table dispatch (random AI, dialog variants, menu routing). Both live in `cop_handlers_flow.asm` alongside flag and wait opcodes.
...
- **Handler:** `SetDungeonKillFlag` @ `extracted/system/engine/cop_handlers_flow.asm:555-566`
...
- **Handler:** `SwitchCase` @ `extracted/system/engine/cop_handlers_flow.asm:571-597`
