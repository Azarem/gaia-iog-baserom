...
_Original analysis of every actor/thinker COP opcode used by the US ROM, derived primarily from the real handlers in `extracted/system/engine/cop_handlers_*.asm`, cross-checked against `db-us/copdef.json` and [Data Crystal — Actor code](https://datacrystal.tcrf.net/wiki/Illusion_of_Gaia/Notes#Actor_code)._
...
- **Does:** Run a BG3 command stream via `ConsoleStringRenderer` (Console overlays, not wide dialogue).
