...
| `$08B464` | `$08B577` | 275 | `daC5_weaver` | actor-def | **Carpet weaver** — body `#26`, init flags `#18`. Loom animation via `JSL ActorDisplayModeSwap` (derives `$24` variant from `$0E` nibble). `$12` bit `$0200`, `SolidHighHere`. **Interact:** `SwitchCase ($24, 4)` — all branches identical: *"She didn't understand. She just kept working."* |
...
| Address | End | Size | Name | Type | Description |
|---------|-----|------|------|------|-------------|
| `$08B577` | `$08B592` | 27 | `camera_delta_oscillator` | actor-def | **Camera vertical nudge** — body `#20` (utility). Each frame: copies `$cameraTargetX` → `$cameraDeltaX`. If `$0036` bit `$0200` set → `$cameraDeltaY++`; else `$cameraDeltaY--`. `SetEntryContinue` loop. Used during pyramid scroll sequences |
