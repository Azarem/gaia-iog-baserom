...
| `func_0AA3A7` | 86 | **Grid-Snap Movement** — Clears hit/dodge callbacks, computes nearest 16×16 grid position, `MoveToward` at speed `$FF`/1 frame, repeats until aligned, then `ResumeAfterSnap`. Installed as deferred resume target by `cop_handlers_collision` when movement is interrupted mid-tile. Not called directly by actor scripts. |
