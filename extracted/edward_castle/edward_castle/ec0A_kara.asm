...
ec0A_kara [
  ...
  code_04CB5D:
    ...
    COP [AnimOnce]
    COP [WaitByte] ( #1D )
    COP [PrintDialogString] ( &dialogstring_04CC84 )
    COP [SetFlagByte] ( #02 )
} >
]
...
code_04CC61 {
    COP [PrintDialogString] ( &dialogstring_04CE45 )
    RTL 
}

code_04CC66 {
    COP [PrintDialogString] ( &dialogstring_04CE4A )
    COP [SetFlagByte] ( #01 )
    RTL 
}

dialogstring_04CC6E `[TPL:9][TPL:1]Kara: Who is it?[PAL:0][PAU:5A][CLD]`

dialogstring_04CC84 `[TPL:9][TPL:1][CLR]Kara: A guest?[PAL:0][END]`

dialogstring_04CC96 `[TPL:B][TPL:1]Kara: [N]You...yesterday...[FIN][TPL:0]Will: I was told to[N]bring the Crystal Ring[N]to King Edward...[PAL:0][END]`

dialogstring_04CCE4 `[TPL:A][TPL:1]Kara: [N]Terrible![N]It's terrible![FIN]Again my father is[N]trying to take something[N]important from someone![FIN]I've escaped from the[N]castle before. Now they[N]won't let me go out![FIN]Recently, something very[N]strange has happened in[N]the castle.[FIN]My mother has hired[N]a famous hunter.[N]It's ominous...[PAL:0][END]`
...
code_04CFA7 {
    ...
    COP [PrintDialogString] ( &dialogstring_04D069 )
    LDA #$0000
    STA $0D60
    LDA #$0001
    STA $0D62
...
dialogstring_04CFDB `[TPL:A][TPL:0]Will: [N](If you want to take her[N]away, now's the time...)[END]`

dialogstring_04D012 `[TPL:A][TPL:1]Karen: Oh, wait![N]It will be a long trip,[N]we should take food![FIN]Would you go to the[N]cellar with me?[PAL:0][END]`

dialogstring_04D069 `[TPL:A][TPL:1]Kara: [N]At last, we leave.[FIN]Let's go to your house.[N]I'm worried about your[N]grandparents.[FIN][DLG:3,6][SIZ:D,3][PAL:0]They hurry to Will's.[END]`