...
code_04969D {
    ...
    COP [AnimLoop]
    COP [PrintWideString] ( &widestring_04983B )
    COP [SetFlagByte] ( #04 )
...
code_049745 {
    ...
    COP [PrintWideString] ( &widestring_049971 )
    COP [SetFlagByte] ( #3E )
    LDA #$CFF0
    TRB $joypadMaskStd
...
code_049796 {
    ...
    COP [StartMusic] ( #19 )
    COP [WaitByte] ( #59 )
    COP [PrintWideString] ( &widestring_049B0E )
    COP [SetEntryContinue]
    SEP #$20
    LDA $APUIO1
    REP #$20
...
code_0497CC {
    COP [PrintWideString] ( &widestring_049802 )
    RTL 
}

code_0497D1 {
    COP [PrintWideString] ( &widestring_0498B6 )
    RTL 
}

code_0497D6 {
    COP [PrintWideString] ( &widestring_049901 )
    LDA #$CFF0
    TSB $joypadMaskStd
    COP [SetFlagByte] ( #0A )
    RTL 
...
code_0497E4 {
    COP [PrintWideString] ( &widestring_049973 )
    RTL 
}

code_0497E9 {
    COP [BranchIfFlagByte] ( #35, #01, &code_0497F9 )
    COP [PrintWideString] ( &widestring_0499D7 )
    COP [GiveItem] ( #09, &code_0497FE )
    RTL 
}

code_0497F9 {
    COP [PrintWideString] ( &widestring_049B6F )
    RTL 
}
...
widestring_049802 `[DEF][TPL:3]ローラ:[N]おや テム. おかえり.[N]夕食まで 時間があるから[N]外で 遊んでおいで.[PAL:0][END]`

widestring_04983B `[TPL:A][TPL:3]ローラ: おーほっほほほーっ.[N]いやだねぇ おまえさん![N]今ごろ そんなことを もちだして.[FIN]そうだ! テム.[N]聞いて おどろいちゃいけないよ.[FIN]さっきまで ここで いっしょに[N]歌っていた 女の子はね···[PAL:0][END]`

widestring_0498B6 `[TPL:B][TPL:3]ローラ: おかえり テム.[N]あたしったら オペラを歌ってたら[N]時の立つのもわすれて···[N]夕ごはん できてないのよ.[PAL:0][END]`

widestring_049901 `[TPL:A][TPL:3]ローラ:[N]エドワード城って言えば[N]地下に 広大な水路があってね.[FIN]その迷宮の水路を 作ったのは[N]何をかくそうこの人なんだよ テム.[FIN][TPL:0]テム:[N]ええっ! ほんとに?[PAL:0][END]`

widestring_049971 `[PAU:40]`

widestring_049973 `[TPL:A][TPL:3]ローラ:[N]さて. かた苦しい話はやめて[N]そろそろ 夕ごはんにしましょ.[FIN]おいしい パイを やいたからね.[N]さあ 二人とも 二階で[N]テーブルに ついていておくれ.[PAL:0][END]`

widestring_0499D7 `[TPL:B][TPL:3]ローラ:[N]おはよう テム.[N]お前に エドワード国王から[N]手紙が 届いているよ.[FIN][PAL:0][DLG:3,6][SIZ:D,4,0]手紙には こんなことが[N]書かれていた.[FIN][TPL:B][TPL:4]オールマンの 所持品である[N]水しょうの指輪を[N]エドワード城まで 持参されたし.[N]        国王 エドワード[FIN][TPL:3]ローラ:[N]この手紙を 見たときから[N]なんだか 悪い虫が さわぐんだよ.[FIN]そうだ. テム.[N]おまじないを ーつ 教えておくわ.[N]困ったときに このメロディーを[N]ふけば きっと のりきれるからね.[FIN]ローラは 不思議なメロディを[N]口ずさんだ.[PAL:0][END]`

widestring_049B0E `[TPL:A][TPL:0][SFX:0][DLY:5]それは すてきな メロディだった.[N][PAU:78][CLR]はじめて 聞いたはずなのに[N]なんだか とっても なつかしい[N]感じがした.[PAU:F0][CLR][PAL:0]ローラの メロディをおぼえた![PAU:FF][CLD]`

widestring_049B6F `[TPL:B][TPL:3]ローラ:[N]気をつけて いくんだよ.[PAL:0][END]`